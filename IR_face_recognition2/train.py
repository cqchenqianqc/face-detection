...
parser.add_argument('--gpus', '-g',type=str, default='0, 1, 2, 3', help='model prefix')
parser.add_argument('--width', '-wi',type = int, default= 32, help='width')
parser.add_argument('--height', '-he',type = int, default= 32, help='height')
parser.add_argument('--resume_weights', '-r',type = str, default= './weights/best_liveness_mobilenetv2-20200903-32.pth.tar', help='Resume weights')
...
for epoch in range(args.epoch):
    ...
    # 保存模型
    torch.save(model.state_dict(), os.path.join(save_model_path, 'mobilenetv2-20200903-32.pth.tar'))
    # 如果是best,则复制最优模型
    if is_best:
        shutil.copyfile(os.path.join(save_model_path, 'mobilenetv2-20200903-32.pth.tar'),
                        os.path.join(save_model_path, 'best_liveness_mobilenetv2-20200903-32.pth.tar'))
